...
Being female is associated with a 6-11% decrease in likelihood of voting for Trump
Accounting for other variables including party and ideology, income doesn't matter in the regression.
...
predict(logistic_fit, new_data = data.frame(gender = "Male", 
                                    age = 37, 
                                    educ = 5, 
                                    race = "Asian", 
                                    income = 8, 
...
If party < 4, then 6% of people in that category voted for Trump. If party >= 4, then 87% of people in that category voted for Trump.
...
cces_tree <- fit(
  update(tree_mod, cost_complexity = 0.1),
  trump ~ income + party + gender + age + educ + race + ideology,
  data = cces
)
...
logistic <- predict(logistic_fit, new_data = cces) %>%
  rename(logistic = .pred_class)

tree <- predict(cces_tree, new_data = cces) %>%
  rename(tree = .pred_class)

forest <- predict(cces_forest, new_data = cces) %>%
  rename(forest = .pred_class)

log_accuracy <- bind_cols(cces, logistic, tree, forest) %>%
  accuracy(trump, logistic)

tree_accuracy <- bind_cols(cces, logistic, tree, forest) %>%
  accuracy(trump, tree)

forest_accuracy <- bind_cols(cces, logistic, tree, forest) %>%
  accuracy(trump, forest)

bind_rows(log_accuracy, tree_accuracy, forest_accuracy) %>%
  mutate(model = c("logistic", "tree", "forest"))
